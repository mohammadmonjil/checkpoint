...
;; Abstract constructors for the initial system pieces

(defstub make-proc-ids () => *)
(defstub make-procs () => *)
(defstub make-channels () => *)



;; Initial snapshot installation
;; sid (snapshot id) for initial snapshot is :init
...
(defun make-cut-meta (sid initiator st)
  (let* ((procs    (procs st))
         ...
         (nbrs     (nbrs-from initiator procs))
         (m        nil)
...
(defun make-recovery-meta (sid initiator st)
  ...
  (let* ((procs                  (procs st))
         (proc-ids               (proc-ids st))
         (initiator-nbrs         (nbrs-from initiator procs))
...
(defun map-proc-to-spec-proc (i p imp-procs)
  ;; Project one implementation process record into a spec process record.
  ;; Keep only the fields needed by the spec.
  (>_ :local-state (local-state p)
      :nbrs-to (nbrs-to i imp-procs)
      :nbrs-from (nbrs-from i imp-procs)))
...
(defun map-procs-to-spec-procs (ids imp-procs)
  ...
  (if (endp ids)
    ...
    (let* ((i          (first ids))
           (imp-p      (g i imp-procs))
           (spec-p     (map-proc-to-spec-proc i imp-p imp-procs))
...
;; (defun map-procs-to-spec-procs (ids imp-procs)
;;   ;; Build the spec process table by projecting each implementation
;;   ;; process down to its local-state only.
;;   (if (endp ids)
;;       nil
;;     (let* ((i         (first ids))
;;            (imp-p     (g i imp-procs))
...
;; (defun rep (imp-st)
;; ;; Map an implementation state to the corresponding spec state.
;; ;; Channels are kept unchanged.
;; ;; Each process keeps only its local-state, neighbors to and from.  
;;   (let* ((ids (proc-ids imp-st))
;;          (imp-procs (procs imp-st)))
;;     (>_ :proc-ids ids
;;         :procs (map-procs-to-spec-procs ids imp-procs)
...
;; ;; ------------------------------------------------------------------
;; ;; Subsection F: Convert untouched original inputs into spec-compatible
;; ;; form and stitch compressed segments from right to left
;; ;;
;; ;; The compressed segment inputs are already built to be spec-level
;; ;; inputs.  However, untouched portions copied directly from the
;; ;; original implementation input sequence may still contain protocol-
...
;; ;;      not be preserved as spec receives.
;; ;;
;; ;;   2. Stitch the final transformed input sequence together from
;; ;;      right to left.
;; ;;
;; ;;      Each compressed segment record replaces the original region
;; ;;      from its checkpoint-start index through its recovery-done
;; ;;      index.  Processing from right to left is important because a
;; ;;      later recovery segment may overlap or subsume an earlier one.
;; ;;      By building the result from the end of the input sequence
;; ;;      backward, later recoveries naturally take precedence, and any
;; ;;      earlier segment whose checkpoint-start lies inside an already
;; ;;      replaced suffix is skipped.
;; ;;
;; ;; Result:
;; ;;   The final output is a spec-compatible input sequence in which
;; ;;   untouched regions are converted to spec-level inputs or :nop,
;; ;;   and checkpoint/recovery regions are replaced by their compressed
;; ;;   segment inputs with correct right-to-left overwrite behavior.
;; ;; ------------------------------------------------------------------
...
;; (defun stitch-compressed-inputs-rtl-aux (inputs trace rev-compressed-seg-records
;;                                                 current-start result-so-far)
;;   ;; Process compressed segment records from right to left.
;;   ;;
;;   ;; current-start = checkpoint-start of the already-built suffix.
;;   ;; result-so-far = transformed suffix starting at current-start.
;;   ;;
;;   ;; If a record has checkpoint-start-index >= current-start, it is
;;   ;; overwritten by a later segment already incorporated into result-so-far.
;;   (if (endp rev-compressed-seg-records)
;;       (append (take-input-range inputs trace 0 current-start)
;;               result-so-far)
;;     (let* ((seg-record      (first rev-compressed-seg-records))
;;            (cp-start        (cis-checkpoint-start-index seg-record))
;;            (recovery-done   (cis-recovery-done-index seg-record))
;;            (compressed-seg  (cis-compressed-seg seg-record)))
;;       (if (>= cp-start current-start)
;;           (stitch-compressed-inputs-rtl-aux inputs
;;                                             trace
;;                                             (rest rev-compressed-seg-records)
;;                                             current-start
;;                                             result-so-far)
;;         (let* ((between-inputs (take-input-range inputs
;;                                                  trace
;;                                                  recovery-done
;;                                                  current-start))
;;                (new-result     (append compressed-seg
;;                                        (append between-inputs
;;                                                result-so-far))))
;;           (stitch-compressed-inputs-rtl-aux inputs
;;                                             trace
;;                                             (rest rev-compressed-seg-records)
...
;; (defun stitch-compressed-inputs-rtl (inputs trace compressed-seg-records)
;;   ;; Build final transformed input list from right to left.
;;   (stitch-compressed-inputs-rtl-aux inputs
;;                                     trace
;;                                     (rev1 compressed-seg-records)
;;                                     (len inputs)
;;                                     nil))
...
;;          (cp-rc-sid-record-list  (collect-recovery-segments-from-trace
;;                                   inputs trace))
;;          (compressed-seg-records (generate-compressed-input-segments
;;                                   inputs trace cp-rc-sid-record-list)))
;;     (stitch-compressed-inputs-rtl inputs
;;                                   trace
;;                                   compressed-seg-records)))
...
;; (defun keep-last-segments-by-checkpoint-start (seg-records)
;;   ;; Later segments with the same checkpoint-start overwrite earlier ones.
;;   ;; Keep only the last record for each checkpoint-start-index.
;;   (if (endp seg-records)
;;       nil
;;     (let* ((seg       (first seg-records))
;;            (rest-kept (keep-last-segments-by-checkpoint-start
...
;; (defun take-input-range-aux (inputs idx stop)
;;   ;; INPUTS is assumed to be aligned with absolute index IDX.
;;   ;; Collect original inputs from absolute index IDX up to STOP-1.
;;   (if (or (endp inputs)
;;           (>= idx stop))
;;       nil
;;     (cons (first inputs)
...
;; (defun take-input-range (inputs start stop)
;;   ;; Collect original inputs from absolute index START up to STOP-1.
;;   (take-input-range-aux (nthcdr start inputs)
;;                         start
;;                         stop))

;; (defun stitch-compressed-inputs (inputs compressed-seg-records idx)
;;   ;; Build final transformed input list starting from absolute index IDX.
;;   ;;
;;   ;; Each record contributes:
;;   ;;   original inputs from IDX to checkpoint-start-1,
;;   ;;   then the compressed segment,
;;   ;;   then continue from recovery-done-index.
;;   ;;
;;   ;; Note: recovery-done-index is treated as the first index AFTER the
;;   ;; covered recovery portion.
;;   (if (endp compressed-seg-records)
;;       (nthcdr idx inputs)
;;     (let* ((seg-record     (first compressed-seg-records))
;;            (cp-start       (cis-checkpoint-start-index seg-record))
;;            (recovery-done  (cis-recovery-done-index seg-record))
;;            (compressed-seg (cis-compressed-seg seg-record)))
;;       (append (take-input-range inputs idx cp-start)
;;               (append compressed-seg
;;                       (stitch-compressed-inputs inputs
;;                                                 (rest compressed-seg-records)
;;                                                 recovery-done))))))

;; (defun compress-inputs (inputs)
;;   (let* ((st0                        (make-initial-state))
;;          (trace                      (run-imp-trace st0 inputs))
;;          (cp-rc-sid-record-list      (collect-recovery-segments-from-trace
;;                                       inputs trace))
...
;; (defun stitch-compressed-inputs-rtl-aux (inputs rev-compressed-seg-records
;;                                                 current-start result-so-far)
;;   ;; Process compressed segment records from right to left.
;;   ;;
;;   ;; current-start = checkpoint-start of the already-built suffix.
...
;;   ;; overwritten by a later segment already incorporated into result-so-far.
;;   (if (endp rev-compressed-seg-records)
;;       (append (take-input-range inputs 0 current-start)
;;               result-so-far)
;;     (let* ((seg-record      (first rev-compressed-seg-records))
;;            (cp-start        (cis-checkpoint-start-index seg-record))
;;            (recovery-done   (cis-recovery-done-index seg-record))
...
;; (defun stitch-compressed-inputs-rtl (inputs compressed-seg-records)
;;   ;; Build final transformed input list from right to left.
;;   (stitch-compressed-inputs-rtl-aux inputs
;;                                     (rev1 compressed-seg-records)
;;                                     (len inputs)
...
;;          (compressed-seg-records (generate-compressed-input-segments
;;                                   inputs trace cp-rc-sid-record-list))
;;          (spec-compliant-inputs  (stitch-compressed-inputs-rtl
;;                                   inputs
;;                                   trace
;;                                   compressed-seg-records)))
