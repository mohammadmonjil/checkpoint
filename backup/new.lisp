...
;; Read the local counter field from a process record.
(defmacro counter (p)
  `(g :counter ,p))
...
;; Read the list of incoming channels still being recorded.
...
;; Create a fresh snapshot entry when the first marker for a snapshot is seen.
...
(encapsulate
 ...
 (local
  ...
  (local
  (defun create-marker-message (sid local-state)
    (declare (ignore local-state nbr))
    nil))
  ...
  (cond ((endp snapshot-ids)
        ...
        (t
         (let* ((sid      (first snapshot-ids))
                ...
                (rec-from (snapshot-recording-from entry)))
...
(defun handle-normal-msg (st i j msg)
  (let* ((procs        (procs st))
         (p            (g i procs))
         (local-state  (local-state p))
	 (local-state  (update-local-state local-state msg j))
         (snapshot-ids (snapshot-ids p))
         (snapshots    (snapshots p))
...
(defun step-start-checkpoint (st i)
  (let* ((procs          (procs st))
         ...
         ;; initiator starts recording all incoming channels
         (recording-from nbrs-from)

         ;; install snapshot entry for this sid
         (entry          (make-snapshot-entry local-state recording-from nil))
         (p              (install-snapshot-entry sid entry p))
         ...
         (msg            (create-marker-message sid local-state))
         (channels       (send-marker-message msg i nbrs-to channels))
...
(defun checkpointing-distributed-system-step (st input)
  (let* (
	...
	; (j (sender input))
         (ttype (ttype input)))
    (case ttype
      (:receive (complete-this))
      (:normal (complete-this))
