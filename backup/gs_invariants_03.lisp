...
;;      process
;; 3.   Use the marker/recovery channel-head invariants to remove
;;      the extra side conditions from step-rcv once they are proved.
...
;; Marker and recovery messages are well-formed only when their sid is already
;; known by at least one process.  Normal messages do not need a sid check.
...
;; General one-channel send lemma for any already-good message.  This reuses the
;; same combined channel-update reasoning developed for compute messages.
...
;; the full channel table.  The two-procs version supports send-after-update cases.
...
(defthm good-channels-p-of-send-msg-all-outgoing-channels-two-procs
  (implies
   (and (good-channels-p srcs dsts channels ids procs1)
        ...
        ;; the new message must be good under the final procs
        (good-msg-p msg ids procs2)
...
(defthm waiting-recovery-from-of-update-proc-for-first-recovery-msg
  (equal (g :waiting-recovery-from
            (update-proc-for-first-recovery-msg p sid j))
         (remove-from-list (g :nbrs-from p) j)))

(defthm subset-of-remove-from-list
...
(defthm proc-status-of-update-proc-for-first-recovery-msg
  (equal (g :proc-status
            (update-proc-for-first-recovery-msg p sid j))
         :recovering))
;end support: first-recovery-process-field-preservation
...
(defthm good-proc-p-of-update-proc-for-first-recovery-msg
  (implies
   (and (good-proc-p p ids)
        ...
        ;; sid must refer to an existing snapshot entry of p.
        (memberp sid (snapshot-ids p))

        ;; j is the sender of the recovery message.
...
;end support: first-recovery-process-goodness
...
(defthm good-channels-p-of-remove-message-from-channel
  ...
  (("Goal"
    :induct (good-channels-p srcs dsts channels ids procs)
    :in-theory (disable remove-message-from-channel))))
;end support: remove-consumed-recovery-message


;start support: recovery-message-goodness
;; A recovery message is good when its sid is present in some process.  In this
;; first-recovery case, the sid is known to be in the receiver's snapshot list.
;; Since the receiver update preserves snapshot ids, the same message is also
;; good under the updated process table.

(defthm good-msg-p-of-get-msg-from-channel-when-recovery
  (implies
   (and (equal (msg-type (get-msg-from-channel j i channels))
               :recovery)
        (memberp (sid (get-msg-from-channel j i channels))
   ...
   (good-msg-p
    ...
    (s i
       (update-proc-for-first-recovery-msg
        ...
        j)
       procs))))
;end support: recovery-message-goodness
...
(defthm snapshot-ids-subset-procs-p-of-update-proc-for-first-recovery-msg-reverse
  ...
  (("Goal"
    :in-theory
    (disable update-proc-for-first-recovery-msg))))
;end support: updated-procs-compatible-with-channel-preservation
...
(defthm good-state-p-of-handle-first-recovery-msg
  (implies
   (and (good-state-p st)
        ...
        ;; The process has the snapshot being recovered.
        (memberp (sid msg)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Marker-message preservation
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Marker handling is the checkpoint-side analogue of recovery handling.
;; A marker message either starts a new snapshot entry at receiver i, or it
;; updates an already-existing snapshot entry by removing sender j from the
;; waiting-marker list.

;start archive: earlier-first-marker-proof-attempts
;; These commented-out events are older proof attempts kept for reference.
;; They are not active ACL2 events.  The active first-marker proof below uses
;; update-proc-for-first-marker-msg and the generic channel-send lemmas instead.
...
;; (defthm good-snapshot-entry-p-of-make-snapshot-entry-remove-from-list
;;   (implies
;;    (true-listp nbrs-from-i)
;;    (good-snapshot-entry-p
;;     (make-snapshot-entry
;;      local-snap-shot
;;      (remove-from-list nbrs-from-i j)
;;      j)
;;     nbrs-from-i)))



;; (defthm good-procs-p-of-install-first-marker-snapshot-entry
;;   (implies
;;    (and
;;     (good-procs-p ids procs ids)
;;     (memberp i ids)
...
;;          (remove-from-list (nbrs-from (g i procs)) j)
;;          j)
;;         (g i procs))
;;        procs)
;;     ids))
;;   :hints
;;   (("Goal"
;;     :induct (good-procs-p ids procs ids)
;;     :in-theory
;;     (disable good-proc-p
;;              install-snapshot-entry
;;              make-snapshot-entry))

;;    ;; Case where the current process in the good-procs-p recursion
;;    ;; is exactly the updated process i.
;;    ("Subgoal *1/2"
;;     :cases ((equal i (car ids))))))

;end archive: earlier-first-marker-proof-attempts


;start support: first-marker-process-field-preservation
;; Field-level facts for the first-marker update.
;; In the first-marker case, receiver i has not seen sid before.  The update
;; installs a new snapshot entry for sid, but it should not change i's neighbor
;; lists, recovery-wait list, counter, or process status.  These rewrite lemmas
;; keep later good-proc-p proofs from opening the updater repeatedly.
...
;end support: first-marker-process-field-preservation


;start support: first-marker-snapshot-entry-preservation
;; The first marker creates a new snapshot entry.  The following lemmas show
;; that the snapshot table remains well formed and that the snapshot-id list
;; remains a true list, unique, and still contains :init after sid is added.
...
;end support: first-marker-snapshot-entry-preservation


;start support: first-marker-status-preservation
;; Receiving a marker does not change the process status.  It only affects the
;; snapshot bookkeeping for the marker's sid.
...
;end support: first-marker-status-preservation


;start support: first-marker-process-wellformedness
;; Lift the field and snapshot-entry facts to good-proc-p and then to
;; good-procs-p for the whole process table.  The first-marker branch requires
;; that sid is new at receiver i and structurally valid.

(defthm good-proc-p-of-update-proc-for-first-marker-msg
  (implies
   (and
    (good-proc-p p ids)
    ;; First-marker case: this process does not already know sid.
    (not (memberp sid (snapshot-ids p)))
    ;; The sid carried by the marker is structurally valid.
    (good-snapshot-id-p sid ids))
   (good-proc-p
    (update-proc-for-first-marker-msg p sid j)
    ids))
...
;end support: first-marker-process-wellformedness


;start support: marker-sid-validity-from-system-knowledge
;; handle-first-marker-msg knows that the marker sid exists somewhere in the
;; system.  To install a new snapshot entry at receiver i, we also need the sid
;; to satisfy good-snapshot-id-p.  These lemmas choose a witness process that
;; already has sid and transfer good-snapshot-id-p from that process's
;; snapshot-id invariant.
...
;; NOTE: This theorem appears a second time in the original file.  If ACL2 does
;; not treat it as a redundant event, delete one copy.
(defthm good-snapshot-ids-p-of-snapshot-ids-when-good-proc-p
  (implies
   (good-proc-p p ids)
   (good-snapshot-ids-p (snapshot-ids p) ids)))
...
;end support: marker-sid-validity-from-system-knowledge


;start support: first-marker-message-goodness-after-update
;; Updating receiver i for the first marker only adds sid to i's snapshot-id
;; list.  Therefore, any marker sid that was known somewhere before the update
;; is still known somewhere afterward, so the marker message remains good under
;; the updated process table.
...
;end support: first-marker-message-goodness-after-update


;start support: first-marker-updated-procs-compatible-with-channel-preservation
;; The generic two-process channel-send theorem needs compatibility between the
;; old procs and the updated procs.  For first-marker handling, outgoing
;; neighbors are unchanged and snapshot ids are monotone: the receiver may gain
;; sid, but no process loses any old snapshot id.
...
;; NOTE: Duplicate of the previous theorem in the original file.  Keep only one
;; copy if ACL2 does not accept the second as redundant.
(defthm snapshot-ids-subset-procs-p-of-update-proc-for-first-marker-msg
  (snapshot-ids-subset-procs-p
   ids
   procs
...
;end support: first-marker-updated-procs-compatible-with-channel-preservation


;start target: good-state-p-of-handle-first-marker-msg
;; Main target theorem for the first marker-message case.
;;
;; handle-first-marker-msg does four important things:
;;   1. removes the consumed marker from channel j -> i;
;;   2. creates a new snapshot entry at receiver i for sid;
;;   3. records i's local state and waits for markers from the other incoming
;;      neighbors;
;;   4. forwards the marker to i's outgoing neighbors.
;;
;; The proof combines receiver-process preservation, removal of the consumed
;; marker, marker-message goodness under the updated procs, and the generic
;; outgoing-send preservation theorem.
...
;end target: good-state-p-of-handle-first-marker-msg


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Non-first marker-message preservation
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; This section handles the case where receiver i already has a snapshot entry
;; for the marker sid.  The handler removes j from that snapshot entry's
;; waiting-marker list.  If no markers remain, the snapshot entry is marked
;; :done; otherwise the snapshot entry stays open.

;start support: non-first-marker-process-field-preservation
;; Field-level facts for update-proc-for-non-first-marker-msg.  The non-first
;; marker update changes only the snapshot entry for sid; it does not change
;; neighbors, recovery waiting state, counter, status, or snapshot-id list.
...
;end support: non-first-marker-process-field-preservation


;start support: non-first-marker-snapshot-entry-preservation
;; Updating an existing marker snapshot either closes the entry by setting
;; status to :done and waiting-marker-from to nil, or removes sender j from the
;; still-waiting marker list.  Both cases preserve good-snapshots-p.
...
;end support: non-first-marker-snapshot-entry-preservation


;start support: non-first-marker-process-wellformedness
;; Lift the non-first marker update to good-proc-p and good-procs-p.  The only
;; extra condition is that receiver i already has sid in its snapshot-id list.
...
(defthm good-proc-p-of-update-proc-for-non-first-marker-msg
  (implies
   (and
    ...
    ;; this process already has an entry for sid.
    (memberp sid (snapshot-ids p)))
   (good-proc-p
    (update-proc-for-non-first-marker-msg p sid j)
    ids))
...
(defthm good-procs-p-of-update-proc-for-non-first-marker-msg
  (implies
   (and
    ...
    ;; sid already exists at process i
    (memberp sid
             (snapshot-ids (g i procs))))
   (good-procs-p
    ids
...
;end support: non-first-marker-process-wellformedness
...
;end support: non-first-marker-updated-procs-field-compatibility
...
;end support: non-first-marker-message-goodness-preservation
...
;end support: non-first-marker-channel-table-preservation


;start target: good-state-p-of-handle-non-first-marker-msg
;; Target theorem for the non-first marker-message handler.
;;
;; Proof idea:
;; 1. remove the consumed marker from channel j -> i;
;; 2. update only the existing snapshot entry for sid at receiver i;
;; 3. show the process table is still good;
;; 4. show existing channel/message well-formedness is not invalidated because
;;    neighbors and snapshot-id lists are unchanged.
...
;end target: good-state-p-of-handle-non-first-marker-msg


;start target: good-state-p-of-handle-marker-msg
;; Dispatcher theorem for marker messages.
;;
;; The handler splits on whether receiver i already knows sid:
;;   1. If not, this is the first-marker case, so the marker sid must be known
;;      somewhere in the system and structurally valid.
;;   2. If yes, this is the non-first marker case, so only the existing
;;      snapshot entry is updated.
...
;end target: good-state-p-of-handle-marker-msg


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Normal-message preservation
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Normal-message handling either ignores the message during recovery waiting,
;; or consumes it and records it into any open channel snapshots.

;start target: good-state-p-of-ignore-normal-msg
;; If receiver i is recovering and still waiting for j, then a normal message
;; from j is ignored by removing it from the channel.  Since no process fields
;; are changed and removing a channel head preserves good-channels-p, the global
...
;end target: good-state-p-of-ignore-normal-msg


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Normal-message-core preservation
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; The core normal-message handler consumes a normal/application message, updates
;; receiver i's local state, and records the message in each open snapshot whose
;; waiting-marker-from still contains sender j.

;start support: normal-msg-core-field-preservation
;; update-proc-for-normal-msg-core does not change neighbors, status, recovery
;; waiting state, counter, or snapshot-id list.  Only local state and the
;; snapshot table may change.
...
;end support: normal-msg-core-field-preservation


;start support: recording-normal-msg-in-open-snapshots
;; Channel snapshots are allowed to store only normal messages.  These lemmas
;; show that appending a normal message to one channel snapshot, and recursively
;; recording it into all open snapshot entries, preserves snapshot well-formedness.
...
;end support: recording-normal-msg-in-open-snapshots


;start target: good-proc-p-of-update-proc-for-normal-msg-core
...
(defthm good-proc-p-of-update-proc-for-normal-msg-core
  (implies
   (and
    (good-proc-p p ids)
    ;; handle-normal-msg-core only records normal/application messages
    ;; into open channel snapshots.
    (equal (msg-type msg) :normal))
   (good-proc-p
    (update-proc-for-normal-msg-core p j msg)
    ids))
...
;end target: good-proc-p-of-update-proc-for-normal-msg-core

;start support: normal-msg-core-process-table-lift
;; Lift the receiver-process preservation theorem to the whole process table.
...
;end support: normal-msg-core-process-table-lift


;start support: normal-msg-core-channel-compatibility
;; The normal-message core update does not change outgoing neighbors or
;; snapshot-id membership.  Therefore, messages/channels that were good under
;; the old process table remain good under the updated process table.
...
;end support: normal-msg-core-channel-compatibility


;start support: normal-msg-core-channel-table-preservation
;; Lift preservation from one channel to channel rows and then to the whole
;; channel table.  Channel contents are not changed here; only the process table
;; used to interpret channel/message well-formedness changes.
...
;end support: normal-msg-core-channel-table-preservation


;start target: good-state-p-of-handle-normal-msg-core
;; Main target theorem for the core normal-message case.
;;
;; The handler consumes the channel head, updates receiver i's local state, and
;; records the normal message in relevant open snapshot channel logs.  The proof
;; combines channel-head removal with the process-table and channel-table
;; preservation lemmas above.
...
(defthm good-state-p-of-handle-normal-msg-core
  (implies
   (and
    ...
    ;; This is really a normal/application message.
    (equal (msg-type msg) :normal))
   (good-state-p
...
;end target: good-state-p-of-handle-normal-msg-core


;start target: good-state-p-of-handle-normal-msg
;; Dispatcher theorem for normal messages.
;;
;; If i is recovering and still waiting for j, the message is ignored.
;; Otherwise, handle-normal-msg-core consumes and records the normal message.
...
;end target: good-state-p-of-handle-normal-msg


;start archive: unused-message-goodness-helper
;; This helper is currently commented out because the active marker/recovery
;; dispatcher proofs use more targeted side conditions.  It may be useful later
;; when strengthening channel-head invariants.

;; (defthm some-proc-has-snapshot-id-p-when-memberp-sid-of-proc
;;   (implies
;;    (and
;;     (memberp i ids)
;;     (memberp sid
...
;end archive: unused-message-goodness-helper


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Receive-step preservation
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; step-rcv dispatches on the type of the head message on channel j -> i.
;; The three dispatcher theorems above handle :normal, :marker, and :recovery.

;start target: good-state-p-of-step-rcv
;; Main receive-step theorem.
;;
;; The two implication hypotheses are the still-needed channel-head invariants:
;;   - a marker at the head of j -> i carries a sid known somewhere;
;;   - a recovery at the head of j -> i carries a sid known by receiver i.
;; Once those invariants are proved from good-state-p or a strengthened state
;; predicate, these side conditions can be removed from this theorem.
...
(defthm good-state-p-of-step-rcv
  (implies
   (and
    ...
    (memberp j (proc-ids st))
    (memberp j (nbrs-from (g i (procs st))))
    ;; The channel head is one of the handled message types.
    (or (equal (msg-type (get-msg-from-channel j i (channels st))) :normal)
        (equal (msg-type (get-msg-from-channel j i (channels st))) :marker)
        (equal (msg-type (get-msg-from-channel j i (channels st))) :recovery))
    ;; Marker-side condition needed by handle-marker-msg:
    ;; if this is a marker at receiver i, then the sid is known
    ...
    (implies
     ...
     (some-proc-has-snapshot-id-p
      ...
      (proc-ids st)
      (procs st)))
    ;; Recovery-side condition needed by handle-recovery-msg:
    ;; then the recovery sid must already be known by receiver i.
    (implies
     (equal (msg-type (get-msg-from-channel j i (channels st))) :recovery)
     (memberp
      (sid (get-msg-from-channel j i (channels st)))
...
;end target: good-state-p-of-step-rcv
