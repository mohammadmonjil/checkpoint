...
;; (defun good-channel-row-p (srcs dst channels ids procs)
;;   (if (endp srcs)
;;       t
;;     (and (good-channel-p (first srcs) dst channels ids procs)
;;          (good-channel-row-p (rest srcs) dst channels ids procs))))

;; (defun good-channels-p (srcs dsts channels ids procs)
;;   (if (endp dsts)
;;       t
;;     (and (good-channel-row-p srcs (first dsts) channels ids procs)
;;          (good-channels-p srcs (rest dsts) channels ids procs))))
...
(defthm good-snapshots-p-of-set-proc-status-crashed
  (implies (good-snapshots-p snapshot-ids
           ...
           (good-snapshots-p snapshot-ids
                             ...
                             ids))
  :otf-flg t
  :hints (("Goal" :do-not '(eliminate-desctructors generalize fertilize)
		  :do-not-induct t)))
...
(defthm good-procs-p-of-crash-update
  (implies (and (true-listp ids)
           ...
           (good-procs-p ids
                         ...
                         all-ids))
    :otf-flg t
  :hints (("Goal" :do-not '(eliminate-destructors generalize fertilize)
		  :do-not-induct t)))

(implies ()
	 (equal (good-procs-p  ids (s i v procs) all-ids)
		(good-procsp  ids procs all-ids)))
