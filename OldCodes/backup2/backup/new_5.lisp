...
(defun good-proc-p (i p procs ids)
  (let* ((nbrs-in  (nbrs-from i procs))
         (nbrs-out (nbrs-to i procs)))
...
(defun good-procs-p (ids procs all-ids)
  (if (endp ids)
    ...
    (let* ((i (first ids))
           (p (g i procs)))
      (and (good-proc-p i p procs all-ids)
...
(defthm checkpoint-recovery-segment-correct
  (let* ((trace      (run-imp-trace sx0 seg-inputs))
         (sid        (list i
                           (counter (g i (procs sx0)))))
         (cut-result
          (scan-until-cut-done seg-inputs
                               trace
                               0
                               sid
                               i))
         (recovery-result
          (scan-until-recovery seg-inputs
                               trace
                               x1
                               sid
                               j))
         ...
         (sx2        (nth x2 trace))
         (compressed (generate-segment-inputs
                      sx0
                      (cut-result-meta cut-result)
                      (recovery-result-meta recovery-result))))
    (implies
     (and (good-state-p sx0)
          (legal-input-sequencep sx0 seg-inputs)

          ;; first input is the checkpoint-start by process i
          (consp seg-inputs)
          (equal (ttype (first seg-inputs)) :start-checkpoint)
          (equal (pid   (first seg-inputs)) i)

          ;; recovery-start occurs at index x1 by process j
          (natp x1)
          (< x1 (len seg-inputs))
          (equal (ttype (nth x1 seg-inputs)) :recover)
          (equal (pid   (nth x1 seg-inputs)) j)

          ;; process j recovers using the same sid
          (equal (car (snapshot-ids
                       (g j (procs (nth x1 trace)))))
                 sid))
     (equal (rep sx2)
            (run-spec (rep sx0)
                      compressed)))))
