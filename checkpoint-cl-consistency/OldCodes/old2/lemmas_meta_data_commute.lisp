...
(defthm
  true-listp-of-swap-first-after-before
...
(defthm
  cl-checkpoint-body-inputs-p-of-swap-first-after-before
...
(defthm
  cl-checkpoint-body-inputs-p-implies-rest
...
(defthm
  legal-input-sequencep-of-cons-if
...
(defthm
  cl-checkpoint-body-inputs-p-implies-first-is-body-input
...
(defthm
  cl-post-pre-swap-start-p-implies-first-post-cut
...
(defthm
  cl-post-pre-swap-start-p-implies-second-pre-cut-at-start
...
(defthm
  cm-cut-not-taken-p-of-after-cut-input-sequence-update
...
(defthm
  cut-markers-in-transit-p-preserved-by-equivalent-state-step
...
;; Projection from CL-POST-PRE-SWAP-START-P needed when expanding
;; NUMBER-OF-BEFORE-CUT-INPUTS in the original POST/PRE order.
(defthm
  cl-post-pre-swap-start-p-implies-second-pre-cut-after-first
...
(defthm
  ...
  (implies
   (and
    ...
    ;; The first input is POST.
    (not
     (cm-cut-not-taken-p
      m
      (pid (first inputs))))

    ;; At least one PRE input occurs in this sequence.
    (< 0
       (number-of-before-cut-inputs
        st m inputs)))
...
;; classification fields of CL-POST-PRE-SWAP-START-P.
;; ------------------------------------------------------------

(defthm
  cl-fixed-post-pre-inversion-is-swappable
  ...
  (implies
   (and
    ...
    ;; The first input is POST at this scan point.
    (not
     (cm-cut-not-taken-p
      m
      (pid (first inputs))))

    ;; The second input remains PRE after the first input.
    (cm-cut-not-taken-p
     (process-cut-step
      (first inputs)
      st
...
(defthm
  cl-post-pre-swap-start-p-implies-complete-cut-classification
...
(defthm
  ...
  (implies
   (and
    (cl-state-equivalent-p st-1 st-2)
...
(defthm
  inversion-count-after-post-pre-pair-commutes
  ...
  (let* (;; ------------------------------------------------------
    ...
    (implies
     (and
      ;; INPUT-POST and INPUT-PRE form a valid adjacent inversion.
      (cl-post-pre-swap-start-p
       st
       m
       input-post
      ...
      ;; POSTFIX is a proper list.
      (true-listp postfix)

      ;; The complete original pair followed by POSTFIX is legal.
      (legal-input-sequencep
       st
       (append
        (list input-post input-pre)
      ...
      ;; The pair and POSTFIX contain checkpoint-body inputs.
      (cl-checkpoint-body-inputs-p
       (append
        (list input-post input-pre)
        postfix)))
  ...
  (("Goal"
    ...
    (;; ---------------------------------------------------------
     ...
     ;; CL-POST-PRE-TWO-PROCESS-CUT-STEPS-COMMUTE equates it with
...
(defthm
  zero-number-of-before-cut-inputs-implies-no-before-cut-inputs
...
(defthm
  zero-inversion-count-implies-before-append-after
...
(defthm
  cl-true-listp-of-append-singleton
...
(defthm
  ...
  (implies
   (and
    ;; The metadata scan begins with no previously recorded inputs.
    (equal
     (cm-before-cut-input-sequence m)
     nil)
    ...
    ;; Hypotheses required by the established reordering theorem.
    (true-listp inputs)
...
(defthm
  before-cut-inputs-agrees-with-fresh-cut-metadata
...
(defthm
  after-cut-inputs-agrees-with-fresh-cut-metadata
...
;; (implies
;;  (cl-checkpoint-complete-segment-p st input-seg)
...
(defthm
  cl-checkpoint-segment-meta-equals-process-cut-segment
  ...
  (("Goal"
   ; :do-not-induct t
    ...
    ;; Expose the public segment/scanner wrappers, but keep the two
    ;; recursive computations opaque. This should produce the precise
    ;; scanner-versus-PROCESS-CUT-SEGMENT bridge checkpoint.
    :in-theory
     (;scan-until-cut-done-aux
					;process-cut-segment
      disable
      process-cut-step
      run-imp-trace
