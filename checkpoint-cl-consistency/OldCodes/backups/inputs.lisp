...
(defthm incoming-channels-equivalent-for-proc-p-of-step-rcv-marker-left
  (implies
   (and
    ...
    ;; The implementation receive consumes a marker message.
    (consp
...
(defthm cut-scan-incoming-channels-equivalent-after-cut-receive-step
  (implies
   (and
    ;; Incoming channels into i already match before this receive step.
    (incoming-channels-equivalent-for-proc-p
    ...
    ;; i is still recording some incoming channels.
    (consp (g i (g :waiting-marker-from m)))

    ;; This theorem is only for receive inputs.
    (equal (g :ttype input) :receive)

    ;; After this receive, i is still not done recording.
    ;; This removes the final-marker case.
    (consp
     (g i
        (g :waiting-marker-from
           (process-cut-receive input st-cur m))))

    ;; Critical extra condition:
    ;; if this receive is for i and the received message is normal,
    ;; then the sender must still be in i's waiting-marker set.
    ;; Therefore process-cut-receive records this input for replay.
    (implies
     (and
      (equal (g :pid input) i)
      (equal
   ...
   ;; Incoming channels into i still match after the implementation
   ;; receive and after replaying the updated cut metadata.
   (incoming-channels-equivalent-for-proc-p
    (g :nbrs-from (g i (g :procs st-cur)))
    i
  ...
  (("Goal"
    ...
    (disable
      ...
      ;; Keep protocol handlers closed unless needed in sublemmas.
      step-rcv
      spec-step
      spec-step-rcv
      handle-normal-msg
      ...
      ;; Keep large predicates closed.
      legal-inputp
      cut-scan-no-recovery-step-p))))



(defun cut-scan-replay-incoming-channels-equivalent-for-proc-p
    (i m st-cur spec-start-st)
  (incoming-channels-equivalent-for-proc-p
...
(defthm cut-scan-replay-incoming-channels-equivalent-after-cut-step
  (implies
   (and
    ;; Incoming channels into i match before replaying this step.
    (cut-scan-replay-incoming-channels-equivalent-for-proc-p
     i m st-cur spec-start-st)

    ;; i has already taken its cut.
    (not (memberp i (cm-cut-not-taken m)))

    ;; i is still in the checkpoint-recording phase.
    ;; This avoids the completed-cut case.
    (consp (cm-waiting-marker-for m i))

    ;; After this input, i is still in the checkpoint-recording phase.
    ;; This rules out the step that consumes the last marker for i.
    (consp
   ...
   ;; Incoming channels into i still match after the step.
   (cut-scan-replay-incoming-channels-equivalent-for-proc-p
    i m-next st-next spec-start-st))
