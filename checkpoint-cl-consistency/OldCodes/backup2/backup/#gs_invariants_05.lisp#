...
(defthm good-msg-list-p-of-cdr
  (implies
   (good-msg-list-p msgs dst ids procs)
   (good-msg-list-p (cdr msgs) dst ids procs)))
...
;; Recovery-message goodness is now receiver-aware.
;;
;; A recovery message on channel j -> i is good only if receiver i already
;; knows the sid carried by the message.  Therefore good-msg-p must be called
;; with destination i.
;;
;; In the first-recovery case, receiver i is updated by
;; update-proc-for-first-recovery-msg.  That update preserves i's snapshot-id
;; list, so the same recovery message remains good for receiver i after the
